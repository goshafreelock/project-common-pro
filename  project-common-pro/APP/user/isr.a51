...
//	extrn xdata (LCDBuff)
    ...
    extrn code (get_adkey_1_value)
#endif
#if defined(TWO_ADKEY_ENABLE)
    extrn data (key_value_2)
    extrn data (ad_key)
#if defined(ADKEY_NEW_METHOD)
    extrn code (get_adkey_2_value)
#endif
    //extrn code (keyScan_ADKEY)
#endif    
...
#if defined(MATRIX_KEY_ENABLE)
    extrn data (matrix_key_value)
#endif

#if OTP_MUSIC_ENABLE
	//extrn code (music_file)
	extrn code (music_file_hello)
	//extrn code (music_file)
#endif
...
timer3isr:
	...
	ANL     T3CON, #~02     ////////////20120505
	CLR   A
	MOV   T3CNTL,A
	MOV   T3CNTH,A
	 ORL     T3CON,#0x2           ;///20120505
	ANL   T3CON,#0BFH
	MOV   R4,#01H
	MOV   R5,#077H
	//LCALL ?C?UIDIV
...
timer3isr_4:
	...
	CJNE 	R7,#01H,timer3isr_5
	SJMP 	?timer3isr?_exit_timer3

timer3isr_5:

	CJNE 	R7,#02H,timer3isr_6
	MOV  	R0,#irBuffer
	MOV  	A,@R0
	ORL  	A,#080H
	MOV  	@R0,A
...
#if defined(TWO_ADKEY_ENABLE)
#if !defined(ADKEY_NEW_METHOD)
    ?PR?keyScan_ADKEY?KEY segment code
    public keyScan_ADKEY
    RSEG  ?PR?keyScan_ADKEY?KEY
keyScan_ADKEY:
       MOV     R7,key_value_2
	//MOV  	R7,ADCDATH
	//ORL  	ADCCON,#080H
...
#if defined(MATRIX_KEY_ENABLE)
       MOV     R7,matrix_key_value
	JNZ  	?C0008
#endif

#ifndef NO_ADKEY_FUNC
#if defined(ADKEY_NEW_METHOD)
    	CALL    get_adkey_1_value
       MOV     A,R7
       CPL     A
   	JNZ     ?C0008
#else   	
