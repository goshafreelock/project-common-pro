...
void alm_time_plus()
{
    if (alm_set==0)
        alm_sw=~alm_sw;
    if (alm_set==1)//if((lcd_x==HOURCOM)||(lcd_x==(LCDCOLUMN-5*8)/2+0))		//curr_alm.hour+=1;
    {
        curr_alm.hour+=1;
        if (curr_alm.hour>23)	curr_alm.hour=0;
    }
    if (alm_set==2)//if((lcd_x==MINCOM)||(lcd_x==(LCDCOLUMN-5*8)/2+24))	//curr_alm.minutes+=1;
    {
        curr_alm.minutes+=1;
...
void alm_time_minus()
{
    ...
    if (alm_set==0)
        alm_sw=~alm_sw;
    if (alm_set==1)	//curr_alm.hour-=1;
    {
        if (curr_alm.hour<1)	curr_alm.hour=24;
        curr_alm.hour-=1;
    }
    if (alm_set==2)	//curr_alm.minutes-=1;
    {
